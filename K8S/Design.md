...
## Pod Estimation
- **Single-node cluster**:
  - Database: 1 pod for PostgreSQL and PVC.
  - Frontend: 2 pod.
  - Backend: 3 pod.
  - Alert Service: 1 pod.
  - Reporting Service: 1 pod.
- **Notes**:
  - The design can be only 1 pod for every container but i assumed that we need HA for front end to make sure it is always up and for the BE so that the app is never fully down.
